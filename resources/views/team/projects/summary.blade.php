...
<head>
    ...
    <title>Payment Summary — {{ $project->name }}</title>
    <style>
        * { box-sizing: border-box; }
        ...
        .toolbar { max-width: 920px; margin: 20px auto 0; display: flex; gap: 10px; justify-content: flex-end; padding: 0 8px; }
...
<body>
    <div class="toolbar">
        <a href="{{ route('team.projects.show', $project->id) }}" class="btn">← Back</a>
        <a href="{{ route('team.projects.summary.excel', $project->id) }}" class="btn btn-success">
    ...
    <div class="sheet">
        <div class="doc-head">
            ...
            <div class="doc-title">
                <h1>Team Payment Summary</h1>
                <div class="muted">Generated {{ $generatedAt }}</div>
            </div>
        </div>

        <div class="meta-grid">
            <div class="row"><span class="k">Project</span><span class="v">{{ $project->name }}</span></div>
            <div class="row"><span class="k">Tracking Code</span><span class="v">{{ $project->code ?? '—' }}</span></div>
            <div class="row"><span class="k">Client</span><span class="v">{{ $project->client->name ?? '—' }}</span></div>
            <div class="row"><span class="k">Members</span><span class="v">{{ $project->teamMembers->count() }} · ৳{{ number_format($share, 2) }} each</span></div>
        </div>

        <div class="stat-row">
            <div class="stat indigo"><div class="l">Project Value</div><div class="v">৳{{ number_format((float) $project->project_value, 2) }}</div></div>
            <div class="stat green"><div class="l">Collected from Client</div><div class="v">৳{{ number_format($collected, 2) }}</div></div>
            <div class="stat amber"><div class="l">Paid to Team</div><div class="v">৳{{ number_format($paidToTeam, 2) }}</div></div>
            <div class="stat"><div class="l">Available</div><div class="v">৳{{ number_format(max(0, $available), 2) }}</div></div>
        </div>

        <h2 class="section">Per-member breakdown</h2>
        <table>
            <thead>
                <tr>
                    <th>Member</th>
            ...
            <tbody>
                ...
                @empty
                    <tr><td colspan="6" style="text-align:center; color:#94a3b8;">No members assigned.</td></tr>
                @endforelse
            </tbody>
            ...
            <tfoot>
                <tr>
                    ...
                    <td class="num">৳{{ number_format($paidToTeam, 2) }}</td>
                    <td class="num">৳{{ number_format($breakdown->sum('remaining'), 2) }}</td>
                    <td></td>
                </tr>
            </tfoot>
            @endif
        ...
        <table>
            ...
            <tfoot>
                <tr><td colspan="5" class="num">Total paid to team</td><td class="num">৳{{ number_format($paidToTeam, 2) }}</td></tr>
            </tfoot>
            @endif
        </table>

        <div class="foot-note">Prime Byte Software Solution · Team payment summary for {{ $project->name }} · Generated {{ $generatedAt }}</div>
    </div>

    <script>
        // If opened with ?print=1, auto-open the print dialog.
